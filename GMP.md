...
- g0/gsignal执行runtime逻辑（调度、GC、信号处理）时，汇编层检查SP：若`SP < stackguard1` ，说明系统栈溢出，直接崩溃；
...
**场景3**：`_Grunning` -> `_Gsyscall` -> `_Grunning`

这条状态转换链路场景为发生系统调用，M与P解绑，然后系统调用结束后G被重新执行。
...
**场景5**：`_Grunning` -> `_Grunnable` -> `_Grunning`（sysmon普通抢占）

sysmon检测到G运行超时后，调用`preemptone()`设置`preempt=true`和`stackguard0=stackPreempt`，G在同步安全点（函数调用边界）或收到SIGURG后执行`asyncPreempt2()`。由于`preemptStop`为false，走`gopreempt_m()`分支，通过`goschedImpl()`将`_Grunning`直接转换为`_Grunnable`并放回全局队列，重新参与调度。入队后与场景2相同，经`schedule() -> execute()`将`_Grunnable`转换为`_Grunning`恢复执行。
...
**场景7**：`_Grunning` -> `_Gcopystack` -> `_Grunning`

发生在栈的扩容期间，`newstack()`中进行状态转换。栈缩容发生在GC阶段，不会经过`_Gcopystack`状态。



**场景8**：`_Grunning` -> `_Gdead` -> `_Grunnable` -> `_Grunning`
...
这部分逻辑在`runtime/preempt.go`中，当进入GC阶段后会执行`markroot()`函数，其中暂停G的逻辑`suspendG()`会把基础状态转变成叠加态。完整的代码如下：
